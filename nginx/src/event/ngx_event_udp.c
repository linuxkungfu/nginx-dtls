...
ngx_event_recvmsg(ngx_event_t *ev)
{
    ...
    do {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ev->log, 0, "recvmsg() data:%d fd:%d", n, lc->fd);
        if (n == -1) {
        ...
        if (c) {
            ...
            if (c->log->log_level & NGX_LOG_DEBUG_EVENT) {
                ...
                ngx_log_debug3(NGX_LOG_DEBUG_EVENT, c->log, 0,
                               "recvmsg: fd:%d n:%z connection:%p", c->fd, n, c);

                c->log->handler = handler;
            }
#endif
            // ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ev->log, 0, "====recvmsg() data:%d connection:%p", n, c);
        ...
        if (c == NULL) {
            ngx_log_debug1(NGX_LOG_DEBUG_EVENT, ev->log, 0, "recvmsg() data:%d ngx_get_connection get connection failed", n);    
            return;
        }
        // ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ev->log, 0, "====recvmsg() data:%d connection:%p", n, c);
        c->shared = 1;
        ...
        *log = ls->log;
        ...
        if (log->log_level & NGX_LOG_DEBUG_EVENT) {
            ...
            ngx_log_debug5(NGX_LOG_DEBUG_EVENT, log, 0,
                           "*%uA recvmsg: %V fd:%d n:%z connection:%p",
                           c->number, &addr, c->fd, n, c);
        }
