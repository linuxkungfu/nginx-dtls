...
ngx_event_udp_accept(ngx_connection_t *c)
{
...
#if (NGX_HAVE_REUSEPORT && (NGX_FREEBSD || NGX_DARWIN))
        on = 1;
    ...
    // ngx_log_debug3(NGX_LOG_DEBUG_EVENT, c->log, 0, "ngx_event_udp_accept connect fd:%d ssl fd:%d connection:%p", c->fd, fd, c);
